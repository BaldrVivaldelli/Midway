...
pub fn with_overlay<'a>(state: &'a Midway, main_content: Element<'a, Message>) -> Element<'a, Message> {
    ...
    stack![main_content, opaque(unsaved_changes_overlay(prompt))].into()
}
...
fn unsaved_changes_overlay(prompt: &crate::app::UnsavedChangesPromptState) -> Element<'_, Message> {
    mouse_area(
        container(opaque(unsaved_changes_box(prompt)))
            .width(Length::Fill)
            .height(Length::Fill)
...
fn unsaved_changes_box(prompt: &crate::app::UnsavedChangesPromptState) -> Element<'_, Message> {
    let mut content = column![text("Esta tab tiene cambios sin guardar. ¿Qué querés hacer?")].spacing(12);

    if let Some(error) = &prompt.error {
        content = content.push(text(error.clone()).color(Color::from_rgb(0.8, 0.2, 0.2)));
    }

    let save_label = if prompt.saving { "Guardando…" } else { "Guardar" };
    ...
    if !prompt.saving {
        save_button = save_button.on_press(Message::RequestComposer(RequestComposerMessage::UnsavedChangesSaveRequested));
        discard_button =
            discard_button.on_press(Message::RequestComposer(RequestComposerMessage::UnsavedChangesDiscardRequested));
        cancel_button =
            cancel_button.on_press(Message::RequestComposer(RequestComposerMessage::UnsavedChangesCancelRequested));
    }

    content = content.push(row![save_button, discard_button, cancel_button].spacing(8));

    container(content).width(Length::Fixed(420.0)).padding(16).style(container::rounded_box).into()
}
